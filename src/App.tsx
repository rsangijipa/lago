...
export default function App() {
  ...
  const handleAudioStatus = useCallback((status: { available: boolean; error: string | null }) => setAudioStatus(status), []);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab' && infoDialogRef.current) {
        const focusable = Array.from(infoDialogRef.current.querySelectorAll(
          'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])',
        ) as NodeListOf<HTMLElement>).filter((element) => !element.hasAttribute('disabled'));
        ...
        >
          ...
          >
            ...
            <ul className="space-y-2 pt-0.5">
              {[
                { color: 'bg-cyan-400',    label: 'Toques na água',      desc: 'Ondas concêntricas que se propagam e amortecem suavemente.' },
                { color: 'bg-amber-400',   label: 'Carpas Koi',          desc: 'Peixes que nadam organicamente e reagem às perturbações.' },
                { color: 'bg-emerald-400', label: 'Nenúfares & Folhas',  desc: 'Vegetação flutuante que oscila com as marolas.' },
                { color: 'bg-blue-400',    label: 'Chuva & Pedras',      desc: 'Gotas suaves ou impacto de seixos atirados.' },
                { color: 'bg-violet-400',  label: 'Cáusticas & Reflexos',desc: 'Refração do leito de pedras e reflexo do céu.' },
                { color: 'bg-rose-400',    label: 'Vento',               desc: 'Rajadas que criam ondas suaves em toda a superfície.' },
